...
; 绝对移动（屏幕坐标）
MouseMoveAbsByKeyMode(keyMode, x, y, speed := 0, isHuman := false) {
    keyMode := Integer(keyMode)
    x := Round(x), y := Round(y)
    speed := Integer(speed)
    SendMode("Event")
    ...
    if (isHuman && keyMode != 3 && keyMode != 4) {
        ...
        hm.SetParams({ IsEnabled: true, Speed: speed })
        hm.Move(x, y)
        return true
    }
    if (keyMode == 3) {
        MC_MoveAbsSmooth(x, y, speed)
        return true
    }
    if (keyMode == 4) {
        AhiMoveAbsSmooth(x, y, speed)
        return true
    }
    MouseMove(x, y, speed)
    return true
...
; 相对移动
MouseMoveRelByKeyMode(keyMode, x, y, speed := 0, isHuman := false) {
    keyMode := Integer(keyMode)
    ...
    if (keyMode == 3) {
        MC_MoveRSmooth(x, y, speed)
        return true
    }
    if (keyMode == 4) {
        AhiMoveRSmooth(x, y, speed)
        return true
    }
...
MouseMoveGameViewByKeyMode(keyMode, x, y, speed := 0) {
    keyMode := Integer(keyMode)
    x := Integer(x), y := Integer(y)
    speed := Integer(speed)
    SendMode("Event")
...
MouseMoveByStrategy(keyMode, moveMode, x, y, speed := 0, clickCount := 0, isHuman := false) {
    keyMode := Integer(keyMode)
    ...
    if (moveMode == 2)
        return MouseMoveGameViewByKeyMode(keyMode, x, y, speed)

    ; AHK：带点击时用 Click 一步完成（含位移），避免相对位移执行两次
    if (clickCount > 0 && keyMode != 3 && keyMode != 4) {
        SetDefaultMouseSpeed(Integer(speed))
        if (moveMode == 1)
    ...
    ok := (moveMode == 1)
        ? MouseMoveRelByKeyMode(keyMode, x, y, speed, isHuman && clickCount <= 0)
        : MouseMoveAbsByKeyMode(keyMode, x, y, speed, isHuman && clickCount <= 0)
    if (!ok)
        return false
...
; clickCount: actionType==3 时的点击次数
SearchMouseActionByStrategy(keyMode, actionType, x, y, speed := 0, clickCount := 1) {
    keyMode := Integer(keyMode)
    ...
    if (actionType == 2)
        return MouseMoveAbsByKeyMode(keyMode, x, y, speed, false)

    n := (actionType == 4) ? 2 : Max(1, Integer(clickCount))
    if (keyMode == 3 || keyMode == 4) {
        if (!MouseMoveAbsByKeyMode(keyMode, x, y, speed, false))
            return false
